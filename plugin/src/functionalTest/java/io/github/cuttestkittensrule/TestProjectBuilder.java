...
public class TestProjectBuilder {
  ...
  public TestProjectBuilder propertyPath(String propertyPath) {
    this.propertyPath =
        Objects.requireNonNull(propertyPath, "Please pass a non-null property path");
    return this;
  }
  ...
  /**
   * Sets if gversion compatibility should be enabled. If this is not specified, the default
  ...
  private FileLocations createFileLocations() {
    ...
    File propertyFile =
        new File(projectDir, Path.of("build", relPath.toArray(String[]::new)).toString());
    return new FileLocations(buildFile, settingsFile, propertyFile);
  }
  ...
  public FileLocations build() throws IOException, InterruptedException {
    ...
    if (propertyPath != null || gVersionCompatibility != null) {
      ...
      if (gVersionCompatibility != null) {
        buildString.append(
            String.format("  gversionBackwardCompatibility = %b%n", gVersionCompatibility));
      }
    ...
    if (!srcFiles.isEmpty()) {
      ...
      for (var entry : srcFiles.entrySet()) {
        ...
        File dirToWrite =
            new File(baseSrc, Path.of(first, components.toArray(String[]::new)).toString());
        if (!dirToWrite.mkdirs()) {
          throw new RuntimeException(
              String.format("Could not create folder for \"%s\"!", entry.getKey()));
        }
    ...
    if (createGitRepo) {
      try {
        ...
        new ProcessBuilder().directory(projectDir).command("git", "init").start().waitFor();
        // add the build.gradle and the settings.gradle files
        new ProcessBuilder().directory(projectDir).command("git", "add", "-A").start().waitFor();
        ...
        Assumptions.abort(
            "Could not execute git commands, so the temporary folder won't be a valid git repository, which would wrongly fail the test!");
